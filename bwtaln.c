...
void bwa_cal_sa_reg_gap(int tid,
                        bwt_t *const bwt,
                        int n_seqs,
                        bwa_seq_t *seqs,
                        const gap_opt_t *opt)
...
void bwa_aln_core(const char *prefix, const char *fn_fa, const gap_opt_t *opt)
{
    ...
    // core loop
    bwa_print_sam_hdr(bns, 0);
    while ((seqs = bwa_read_seq(ks, 0x200000, &n_seqs, opt->mode, opt->trim_qual)) != 0) {
        tot_seqs += n_seqs;
        t = clock();
