...
int bwa_aln2seq_core(int n_aln,
                    ...
                    int *strand)
{
    bwtint_t pos_f;
    int is_rev;
    // initialise strand to 0 otherwise we could return without setting it
    *strand = 0;
    // position on the forward-reverse coordinate
    pos_f = bwt_sa(bwt, sapos);
    if (pos_f < bns->l_pac && bns->l_pac < pos_f + ref_len)
        return (bwtint_t)-1;
    // position on the forward strand; this may be the first base or the last base
    pos_f = bns_depos(bns, pos_f, &is_rev);
    *strand = !is_rev;
    if (is_rev)
        // position of the first base
        pos_f = pos_f + 1 < ref_len? 0 : pos_f - ref_len + 1;
    return pos_f;
    // FIXME: it is possible that pos_f < bns->anns[ref_id].offset
}
                      ...
                      int n_threads)
{
    ...
    for (i = 0; i != n_seqs; ++i) {
        ...
        if (i % n_threads != tid) continue;
        #endif
        // IMPORTANT: s->seq is reversed here!!!
                    ...
                    int max_top2)
{
    ...
    if (p->type != BWA_TYPE_NO_MATCH || (mate && mate->type != BWA_TYPE_NO_MATCH)) {
        ...
        if (p->type != BWA_TYPE_NO_MATCH) {
            ...
            err_printf("\tXM:i:%d\tXO:i:%d\tXG:i:%d",
                       p->n_mm, p->n_gapo, p->n_gapo+p->n_gape);
...
void bwa_sai2sam_se_core(const char *prefix,
                         const char *fn_sa,
                         const char *fn_fa,
                         int n_occ,
                         const char *rg_line)
{
    ...
    if (strncmp(magic, SAI_MAGIC, 4) != 0) {
        fprintf(stderr,
                "[E::%s] Unmatched SAI magic. Please re-run `aln' with the same version of bwa.\n", __func__);
        exit(1);
    }
    ...
    while ((seqs = bwa_read_seq(ks, 0x40000, &n_seqs, opt.mode, opt.trim_qual)) != 0) {
        ...
        for (i = 0; i < n_seqs; ++i) {
            ...
            //Multiple hits are reconstructed here
            occ += bwa_aln2seq_core(n_aln, aln, p, 1, n_occ);
        }
        ...
        //Julian MD tag is set here
        occ = bwa_refine_gapped(0, bns, n_seqs, seqs, 0, 0);
        fprintf(stderr, "%.2f sec\n", (float)(clock() - t) / CLOCKS_PER_SEC); t = clock();
        fprintf(stderr, "[JULIAN %s] %d extra hits\n", __func__, occ);
